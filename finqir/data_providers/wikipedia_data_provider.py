...
class WikipediaDataProvider(BaseDataProvider):
    ...
    https://bilgin-kocak.github.io/finqir/tutorials/11_time_series.html
    """
    ...
    def run(self) -> None:
        ...
        Raises:
            FinQIRError: If there is an invalid Nasdaq Data Link token,
        ...
        for _, ticker_name in enumerate(self._tickers):
            ...
            try:
                ...
                raise FinQIRError("Nasdaq Data Link invalid token.") from ex
            except nasdaqdatalink.LimitExceededError as ex:
                logger.debug(ex, exc_info=True)
                raise FinQIRError("Nasdaq Data Link limit exceeded.") from ex
                ...
                logger.debug(ex, exc_info=True)
                raise FinQIRError(f"Nasdaq Data Link Error for '{name}'.") from ex

            try:
                self._data.append(stock_data["Adj. Close"])
            except KeyError as ex:
                raise FinQIRError("Cannot parse Nasdaq Data Link output.") from ex

        if stocks_notfound:
            raise FinQIRError(f"Stocks not found: {stocks_notfound}. ")
